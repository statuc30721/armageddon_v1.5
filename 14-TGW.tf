...
# Attach transit gateway to Tokyo VPC A.
...
#                                                                     #
#=====================================================================#
# Need to change this setup to avoid complexity!!!

# Transit Gateway CYBER Tokyo VPC

resource "aws_ec2_transit_gateway" "VPC-Cyber-Tokyo-Test-TGW01" {
    auto_accept_shared_attachments = "enable"
   #default_route_table_association = "enable"
   #default_route_table_propagation = "enable"
    dns_support = "enable"
    description = "tokyo CYBER VPC transit gateway"
    provider = aws.tokyo

    tags = {
      Name = "Tokyo CYBER TGW"
    }
}
...
resource "aws_ec2_transit_gateway_vpc_attachment" "VPC-Cyber-Tokyo-Test-TGA" {
    subnet_ids = [aws_subnet.private-cyber-ap-northeast-1a.id]
    transit_gateway_id = aws_ec2_transit_gateway.VPC-Cyber-Tokyo-Test-TGW01.id
    vpc_id = aws_vpc.VPC-Cyber-Tokyo-Test.id
    provider = aws.tokyo
    
    tags = {
      Name = "VPC-Cyber-Tokyo-Test-TGA"
    }
}
...
resource "aws_ec2_transit_gateway_peering_attachment" "NY-to-CYBER-Tokyo" {
    ...
    peer_transit_gateway_id = aws_ec2_transit_gateway.VPC-Cyber-Tokyo-Test-TGW01.id
...
# We want Tokyo CYBER Transit Gateway to accept peer request from New York VPC.
...
# Create STATIC route from  Tokyo CYBER VPC to New York VPC.

resource "aws_ec2_transit_gateway_route" "TokyoCYBER-NewYork" {
    provider = aws.tokyo
    destination_cidr_block         = "10.21.0.0/16"
    transit_gateway_attachment_id  = aws_ec2_transit_gateway_peering_attachment_accepter.Accept-NY-Peer.id
    transit_gateway_route_table_id = aws_ec2_transit_gateway.VPC-Cyber-Tokyo-Test-TGW01.association_default_route_table_id
}
...
resource "aws_ec2_transit_gateway_vpc_attachment" "VPC-C-London-TGA01" {
    subnet_ids = [aws_subnet.private-eu-west-2a.id]
...
# Create STATIC route from  Tokyo CYBER VPC to Sao Paulo VPC.
...
resource "aws_ec2_transit_gateway_vpc_attachment" "VPC-E-AUS-TGA01" {
    subnet_ids = [aws_subnet.private-ap-southeast-2a.id]
...
# Create STATIC route from  Tokyo CYBER VPC to Australia VPC.
...
# Create STATIC route from  Tokyo CYBER VPC to Hong Kong VPC.
...
resource "aws_ec2_transit_gateway_vpc_attachment" "VPC-G-California-TGA01" {
    subnet_ids = [aws_subnet.private-us-west-1a.id]
...
# Create STATIC route from  Tokyo CYBER VPC to California VPC.
